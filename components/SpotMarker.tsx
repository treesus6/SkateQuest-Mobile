...
const SPOT_COLORS: Record<string, string> = {
  park: '#10b981', street: '#3b82f6', diy: '#f59e0b', quest: '#8b5cf6', shop: '#ef4444',
};
...
export default function SpotMarker({ spotType, hasBondoAlert = false, crewColor, size = 40 }: SpotMarkerProps) {
  ...
  }, [hasBondoAlert, pulseAnim]);
  const Icon = SPOT_ICONS[spotType] || MapPin;
  const backgroundColor = hasBondoAlert ? '#ef4444' : (crewColor || SPOT_COLORS[spotType] || '#d2673d');
  return (
    <Animated.View style={{ backgroundColor, width: size, height: size, borderRadius: size / 2, justifyContent: 'center', alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.3, shadowRadius: 4, elevation: 5, transform: [{ scale: hasBondoAlert ? pulseAnim : 1 }] }}>
      <Icon color="#fff" size={size * 0.45} />
      {hasBondoAlert && (
        <View className="absolute -top-1 -right-1 bg-white rounded-full justify-center items-center" style={{ width: 20, height: 20 }}>
