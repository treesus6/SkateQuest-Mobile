...
export const Toast: React.FC = () => {
  ...
  const hide = useCallback(() => {
    ...
    ]).start(() => { setVisible(false); setConfig(prev => { prev?.onDismiss?.(); return null; }); });
  }, [fadeAnim, slideAnim]);

  const show = useCallback((newConfig: ToastConfig) => {
    setConfig(newConfig); setVisible(true);
    Animated.parallel([
  ...
  useEffect(() => { toastInstance = show; return () => { toastInstance = null; }; }, [show]);
  ...
  return (
    <Animated.View style={{ position: 'absolute', top: Platform.OS === 'ios' ? 50 : 20, left: 10, right: 10, borderRadius: 8, padding: 16, backgroundColor, opacity: fadeAnim, transform: [{ translateY: slideAnim }], shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 3.84, elevation: 5, zIndex: 9999 }}>
      <View className="flex-row items-center">
        <Icon color="#fff" size={20} />
        <Text className="flex-1 text-white text-sm leading-5 mx-3" numberOfLines={3}>{config.message}</Text>
        <TouchableOpacity onPress={hide} className="p-1"><X color="#fff" size={18} /></TouchableOpacity>
      </View>
    </Animated.View>
  );
};
