-- Defense-in-depth hardening for the atomic session RSVP RPC.
--
-- The original function used `auth.uid()::text != p_user_id`, which does not
-- reject anonymous calls because NULL comparisons evaluate to NULL rather than
-- TRUE. Keep the function authenticated-only and make the body itself null-safe.
...
begin
  ...
  return jsonb_build_object(
    'is_attending', not v_is_attending,
    'attendee_count', v_new_count
  );
end;
$$;
