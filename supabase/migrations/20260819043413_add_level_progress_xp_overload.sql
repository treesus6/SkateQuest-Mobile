create or replace function public.get_level_progress(p_user_xp integer)
returns jsonb
language plpgsql
immutable
security invoker
...
as $$
declare
  user_xp integer := greatest(0, coalesce(p_user_xp, 0));
...
revoke all on function public.get_level_progress(integer) from public, anon;
grant execute on function public.get_level_progress(integer) to authenticated, service_role;
