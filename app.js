...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function setActiveButton(activeBtn) {
    if (!activeBtn) return;
    [discoverBtn, addSpotBtn, crewsBtn, eventsBtn, shopsBtn, charityBtn, profileBtn, legalBtn]
      .filter(btn => btn)
  ...
  if (charityBtn) {
    charityBtn.onclick = async () => {
      ...
      try {
        ...
        content.innerHTML =
          '<div style="padding:1rem;"><h2>Error</h2><p>Failed to load charity module. Please refresh the page.</p></div>';
      }
    };
  }
  ...
  async function renderChallenges() {
    ...
    try {
      ...
      if (allChallenges.length === 0) {
        challengesList.innerHTML =
          '<p style="color: #888; text-align: center; padding: 20px;">No challenges available yet. Add spots with challenges to get started!</p>';
        return;
      }

      // Filter out challenges already completed by current user
      const pendingChallenges = allChallenges.filter(
        c => !c.completedBy || !c.completedBy.includes(currentUserId)
      );

      if (pendingChallenges.length === 0) {
        challengesList.innerHTML =
          '<p style="color: #888; text-align: center; padding: 20px;">You\'ve completed all available challenges! Great job!</p>';
        return;
      }

      // Render challenges
      challengesList.innerHTML = pendingChallenges
        .map(challenge => {
      ...
      challengesList.innerHTML =
        '<p style="color: #f44336; text-align: center; padding: 20px;">Error loading challenges. Please try again.</p>';
    }
  }
