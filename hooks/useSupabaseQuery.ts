...
interface UseSupabaseQueryOptions { cacheKey?: string; cacheTTL?: number; retries?: number; enabled?: boolean; persist?: boolean; }
interface UseSupabaseQueryResult<T> { data: T | null; loading: boolean; error: string | null; refetch: () => Promise<void>; isStale: boolean; }
...
export function useSupabaseQuery<T>(
  ...
  const { cacheKey, cacheTTL = DEFAULT_CACHE_TTL, retries = MAX_RETRIES, enabled = true, persist = false } = options;

  const [data, setData] = useState<T | null>(() => {
    if (cacheKey) { const cached = cache.get(cacheKey); if (cached && Date.now() - cached.timestamp < cacheTTL) return cached.data as T; }
    return null;
  });
    ...
    try {
      ...
      if (queryError) {
        ...
        if (cacheKey && result) {
          ...
          if (persist) PersistentCache.set(cacheKey, result, cacheTTL);
        }
      }
    } catch (err: unknown) {
      if (!mountedRef.current) return;
  ...
  const fetch = useCallback(async () => {
    ...
    if (mountedRef.current) { setLoading(false); setIsStale(false); }
  }, [fetchWithRetry, enabled]);

  useEffect(() => { fetch(); }, [fetch]);
  useEffect(() => {
    if (!cacheKey || !data) return;
    const timer = setTimeout(() => { if (mountedRef.current) setIsStale(true); }, cacheTTL);
...
export function invalidateCache(key?: string) {
  if (key) { cache.delete(key); PersistentCache.remove(key); }
  else { cache.clear(); PersistentCache.clearAll(); }
}
