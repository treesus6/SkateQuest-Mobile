...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  const apiFetch = async (endpoint, options) => {
    ...
    try {
      if (
        ...
        window.firebaseInstances.auth.currentUser.getIdToken
      ) {
        const token = await window.firebaseInstances.auth.currentUser.getIdToken(
          /* forceRefresh */ false
        );
  ...
  async function renderBadgesForUser(uid) {
    try {
      ...
      console.error('renderBadgesForUser', e);
    }
  }
  ...
  if (issueChallengeButton) {
    issueChallengeButton.addEventListener('click', async () => {
      ...
      try {
        ...
        if (
          window.firebaseInstances &&
          window.firebaseInstances.addDoc &&
          window.firebaseInstances.collection
        ) {
        ...
        showToast(
          'Failed to create challenge: ' + (err && err.message ? err.message : 'unknown error'),
          'error'
        );
      } finally {
  ...
  if (ratingBtn) {
    ratingBtn.addEventListener('click', async () => {
      ...
      if (!ratingSelect) {
        showToast(
          'Rating select element not found. Please reload the page or contact support.',
          'error'
        );
        return;
      }
    ...
    try {
      ...
      snaps.forEach(d => {
        ...
        if (s.location && s.location.lat && s.location.lng)
          heatData.push([s.location.lat, s.location.lng, s.visits || 1]);
      });
  ...
  if (heatToggle)
    heatToggle.addEventListener('change', e => {
      if (e.target.checked) renderHeatmap();
      else if (heatLayer) {
    ...
    try {
      ...
      parksMarkers = parks
        .map(p => {
          ...
          const m = L.marker([p.lat, p.lng], { title: p.name }).bindPopup(
  ...
  async function callCompleteChallengeFunction(challengeId) {
    try {
      if (
        ...
        return null;
      const fn = window.firebaseInstances.httpsCallable(
        window.firebaseInstances.functions,
        'completeChallenge'
      );
    ...
    try {
      ...
      if (runTransaction) {
        await runTransaction(db, async tx => {
          ...
          const extra = {};
          try {
            if (window.firebaseInstances.serverTimestamp)
              extra.completedAt = window.firebaseInstances.serverTimestamp();
          } catch (e) {}
  ...
  if (completeBtn) {
    completeBtn.addEventListener('click', async () => {
      const conf = document.getElementById('challenge-confirm');
      if (!conf || !conf.dataset || !conf.dataset.challengeId)
        return alert('No challenge selected or challenge not created from this device.');
      ...
      try {
        if (
          ...
          if (window.firebaseInstances && window.firebaseInstances.signInAnonymously) {
            await window.firebaseInstances
              .signInAnonymously(window.firebaseInstances.auth)
              .catch(() => {});
          }
        }
        const userId =
          ...
          window.firebaseInstances.auth &&
          window.firebaseInstances.auth.currentUser
            ? window.firebaseInstances.auth.currentUser.uid
            : null;
        ...
        if (
          window.firebaseInstances &&
          window.firebaseInstances.getDoc &&
          window.firebaseInstances.doc
        ) {
        ...
        console.error('complete button handler', e);
        showToast('Failed to mark challenge complete', 'error');
      }
    });
  }
  ...
  function subscribeLeaderboardRealtime() {
    try {
      ...
      console.error('subscribeLeaderboardRealtime', e);
    }
  }
  ...
  function subscribeCurrentUserRealtime(uid) {
    try {
      ...
      console.error('subscribeCurrentUserRealtime', e);
    }
  }
    ...
    try {
      ...
      pendingUnsub = onSnapshot(
        ...
        snap => {
          ...
          list.innerHTML = '';
          if (snap.empty) {
            list.textContent = 'No pending challenges';
            return;
          }
          snap.forEach(ch => {
            const data = ch.data();
                ...
                if (modal && modalText && modalTitle) {
                  ...
                  window.__pendingConfirmAction = async () => {
                    try {
                      ...
                      const uid =
                        window.firebaseInstances.auth && window.firebaseInstances.auth.currentUser
                          ? window.firebaseInstances.auth.currentUser.uid
                          : null;
                      if (!uid) {
                        alert('Sign in required to complete challenge');
                        return;
                      }
                      await completeChallenge(ch.id, uid);
                      await updateStreak(uid);
                      const uRef = window.firebaseInstances.doc(
                      ...
                      console.error('confirm complete', e);
                      showToast('Failed to complete challenge', 'error');
                    }
                  };
                }
              } catch (e) {
                console.error('pending complete', e);
                showToast('Failed to open confirm', 'error');
              }
            });
            row.appendChild(txt);
            row.appendChild(btn);
      ...
      console.error('subscribePendingChallenges', e);
    }
  }
    ...
    if (
      ...
      try {
        if (
          ...
          window.firebaseInstances.signInAnonymously(window.firebaseInstances.auth).catch(e => {
            console.debug('anonymous sign-in failed', e);
          });
        }
      } catch (e) {
        console.debug('signInAnonymously check failed', e);
      }
  ...
  async function uploadProof() {
    ...
    try {
      ...
      try {
        ...
        showToast(
          'Location is required for proof upload. Please enable location permissions.',
          'error'
        );
  ...
  // Initialize the feed
  Promise.all([loadChallengeFeed(), initializeTestData()]).catch(console.error);
