...
export default function HomeScreen() {
  ...
  const loadData = useCallback(async () => {
    ...
    const [profileRes, liveRes, questRes, clipRes, bountyRes] = await Promise.all([
      ...
      supabase
        .from('skatetv_clips')
        .select('id,title,thumbnail_url,video_url,likes,views,trick_name,park_name,created_at,profiles(username)')
        .order('created_at', { ascending: false })
        .limit(8),
      supabase
        .from('bounties')
        .select('id,trick_name,park_name,xp_reward,status,expires_at')
        .eq('status', 'open')
        .order('xp_reward', { ascending: false })
        .limit(4),
    ]);
  ...
  }, [drift, intro, livePulse, loadData]);
  ...
  const progress = Math.max(0, Math.min(1, (xp - xpForCurrent) / Math.max(1, xpForNext - xpForCurrent)));
    ...
    <SafeAreaView style={s.container} edges={['top']}>
      ...
      >
        ...
        >
          ...
          <View style={s.scoreStrip}>
            ...
            <View style={s.scoreMain}>
              <View style={s.scoreTop}>
                ...
                <Text style={s.scoreNext}>{Math.max(0, xpForNext - xp).toLocaleString()} to next</Text>
              </View>
          ...
          {firstQuest ? (
            <Pressable style={s.questHero} onPress={() => go('DailyQuests')}>
              ...
              <View style={s.questXpBlock}>
                ...
                <ArrowUpRight color={INK} size={19} style={{ marginTop: 12 }} />
              </View>
            </Pressable>
          ) : (
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.clipRail}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.liveRail}>
            {checkins.length > 0 ? checkins.map((item, index) => (
              ...
              <Pressable style={s.emptyLive} onPress={() => go('LiveCheckIn')}>
                ...
                <Text style={s.emptyLiveTitle}>Nobody checked in yet.</Text>
                <Text style={s.emptyLiveText}>Start the session and put your spot on the live scene.</Text>
              </Pressable>
            )}
          </ScrollView>
              ...
              <View style={s.bountyStack}>
                {bounties.slice(0, 3).map((bounty, index) => (
                  <Pressable key={bounty.id} style={[s.bountyTicket, index === 0 && s.bountyTicketLead]} onPress={() => go('BountyBoard')}>
                    ...
                    <View style={s.bountyReward}>
                      <Trophy color={INK} size={15} />
                      <Text style={s.bountyXp}>{bounty.xp_reward} XP</Text>
                    </View>
                  </Pressable>
                ))}
              </View>
    ...
    <View style={s.sectionHeader}>
      ...
      {action && onPress ? (
        <Pressable onPress={onPress} style={s.sectionAction}>
          ...
          <ChevronRight color={ORANGE} size={15} />
        </Pressable>
      ) : null}
    </View>
  );
...
function ActionTile({ accent, icon, label, sub, onPress }: { accent: string; icon: React.ReactNode; label: string; sub: string; onPress: () => void }) {
  return (
    <Pressable style={[s.actionTile, { backgroundColor: accent }]} onPress={onPress}>
...
const s = StyleSheet.create({
  ...
  levelStamp: { width: 86, backgroundColor: ORANGE, alignItems: 'center', justifyContent: 'center', paddingVertical: 14, transform: [{ rotate: '-1deg' }] },
  ...
  actionLabel: { color: INK, fontSize: 16, fontWeight: '900', letterSpacing: -0.2 },
  actionSub: { color: 'rgba(7,8,11,0.7)', fontSize: 9, fontWeight: '800', marginTop: 2 },
  clipRail: { paddingHorizontal: 14, gap: 12 },
  ...
  exploreLabel: { color: PAPER, fontSize: 13, fontWeight: '900', marginTop: 16, letterSpacing: 0.2 },
