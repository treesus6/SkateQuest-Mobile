...
export default function SkateTVLiveScreen() {
  ...
  const toggleLike = async (clip: Clip) => {
    ...
    setLikedClips(current => {
      ...
      if (wasLiked) next.delete(clip.id);
    ...
    if (error) {
      ...
      setClips(current =>
        current.map(item => (item.id === clip.id ? { ...item, likes: oldLikes } : item))
      );
    ...
    if (result) {
      ...
      if (typeof result.likes === 'number') {
        setClips(current =>
          current.map(item =>
            item.id === clip.id ? { ...item, likes: result.likes as number } : item
          )
        );
      }
    }
  ...
  const watchClip = async (clip: Clip) => {
    ...
    if (typeof result?.views === 'number') {
      setClips(current =>
        current.map(item =>
          item.id === clip.id ? { ...item, views: result.views as number } : item
        )
      );
    }
    void Linking.openURL(clip.video_url).catch(() =>
      Alert.alert('Could not open clip', 'Try again in a moment.')
    );
  };
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View>
              ...
              <View style={s.heroTop}>
                <View>
                  <View style={s.kickerRow}>
                    <Video size={14} color={ORANGE} />
                    <Text style={s.kicker}>SKATEQUEST VIDEO</Text>
                  </View>
            ...
            <View style={s.tabs}>
              <Pressable
                style={[s.tab, activeTab === 'fresh' && s.tabOn]}
              ...
              <Pressable
                style={[s.tab, activeTab === 'featured' && s.tabOn]}
          ...
          return (
            <View style={s.card}>
              <Pressable style={s.media} onPress={() => void watchClip(item)}>
                {item.thumbnail_url ? (
                  <Image source={{ uri: item.thumbnail_url }} style={s.fill} contentFit="cover" />
                ) : (
                  <View style={s.noThumb}>
                    <Camera color={ORANGE} size={42} />
                    <Text style={s.noThumbText}>VIDEO READY</Text>
                  </View>
                )}
                <View style={s.scrim} />
                <View style={s.indexSticker}>
                  <Text style={s.indexText}>{String(index + 1).padStart(2, '0')}</Text>
                </View>
                {item.featured ? (
                  <View style={s.featured}>
                    <Flame color={INK} size={12} />
                ...
                <View style={s.mediaCopy}>
                  <Text style={s.clipTitle} numberOfLines={2}>
                    {item.title || item.trick_name || 'Skate clip'}
                  </Text>
              ...
              <View style={s.info}>
                <View style={s.tagRow}>
                  {item.trick_name ? (
                    <View style={s.trickTag}>
                      <Text style={s.trickTagText}>{item.trick_name.toUpperCase()}</Text>
                    </View>
                  ) : null}
                  {item.park_name ? (
                    <View style={s.spotTag}>
                      <MapPin color={MUTED} size={12} />
                      <Text style={s.spotText} numberOfLines={1}>
                        {item.park_name}
                      </Text>
                    </View>
                  ) : null}
                ...
                <View style={s.actions}>
                  <Pressable
                    style={[s.action, liked && s.actionLiked]}
                    onPress={() => void toggleLike(item)}
                  >
                    <Heart
                      color={liked ? INK : ORANGE}
                  ...
                  <View style={s.action}>
                    ...
                    <Text style={s.actionText}>{item.views}</Text>
                  </View>
        ...
        ListEmptyComponent={
          <Pressable style={s.empty} onPress={() => setUploadModal(true)}>
            <View style={s.emptyIcon}>
              <Upload color={INK} size={28} />
            </View>
            <Text style={s.emptyTitle}>
              {activeTab === 'featured' ? 'NO FEATURED CLIPS YET' : 'THE FEED IS EMPTY'}
            </Text>
            <Text style={s.emptyText}>
              {activeTab === 'featured'
                ? 'Switch to Fresh or post something worth featuring.'
                : 'Be the first real clip on the scene.'}
            </Text>
          </Pressable>
        }
      />
        ...
        <View style={s.modalShade}>
          <View style={s.modalSheet}>
            <View style={s.modalHeader}>
              <View>
                <Text style={s.kicker}>DROP A CLIP</Text>
                <Text style={s.modalTitle}>Post to SkateTV</Text>
              </View>
              <Pressable style={s.close} onPress={() => setUploadModal(false)}>
                <X color={PAPER} size={18} />
              </Pressable>
            </View>
            <ScrollView contentContainerStyle={s.modalContent} keyboardShouldPersistTaps="handled">
              <Pressable
                style={[s.videoPicker, uploadVideo && s.videoPickerReady]}
                onPress={() => void pickVideo()}
              >
                <Upload color={uploadVideo ? INK : ORANGE} size={29} />
                <Text style={[s.videoPickerTitle, uploadVideo && s.videoPickerReadyText]}>
                  {uploadVideo ? 'VIDEO LOCKED IN' : 'CHOOSE SKATE CLIP'}
                </Text>
                <Text style={[s.videoPickerSub, uploadVideo && s.videoPickerReadySub]}>
                  Up to 60 seconds
                </Text>
              </Pressable>
              <Input
              ...
              />
              <Pressable
                style={[s.postButton, (!uploadVideo || uploading) && s.postButtonDisabled]}
              ...
              >
                {uploading ? (
                  ...
                  <>
                    <Upload color={INK} size={18} />
                    <Text style={s.postButtonText}>POST REAL CLIP</Text>
                  </>
                )}
              </Pressable>
            </ScrollView>
