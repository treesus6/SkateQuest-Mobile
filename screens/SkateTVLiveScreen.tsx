...
import { ArrowUpRight, Camera, Eye, Flame, Heart, MapPin, Play, Plus, Sparkles, Upload, Video, X } from 'lucide-react-native';
...
export default function SkateTVLiveScreen() {
  ...
  const toggleLike = async (clip: Clip) => {
    ...
    const oldLikes = clip.likes;
    setLikedClips(current => {
      const next = new Set(current);
      if (wasLiked) next.delete(clip.id); else next.add(clip.id);
      return next;
    });
    setClips(current => current.map(item => item.id === clip.id ? { ...item, likes: Math.max(0, item.likes + (wasLiked ? -1 : 1)) } : item));

    const { data, error } = await supabase.rpc('toggle_skatetv_like', { p_clip_id: clip.id });
    if (error) {
      setLikedClips(current => {
        const next = new Set(current);
        if (wasLiked) next.add(clip.id); else next.delete(clip.id);
        return next;
      });
      setClips(current => current.map(item => item.id === clip.id ? { ...item, likes: oldLikes } : item));
    ...
    if (result) {
      setLikedClips(current => {
        const next = new Set(current);
        if (result.liked) next.add(clip.id); else next.delete(clip.id);
        return next;
      });
  ...
  return (
    <SafeAreaView style={s.container} edges={['top']}>
      <FlatList
        ...
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); void loadClips(); }} tintColor={ORANGE} />}
        ListHeaderComponent={
          <View>
            <View style={s.hero}>
              <View style={s.heroSlashA} /><View style={s.heroSlashB} />
              <View style={s.heroTop}>
                ...
                <Pressable style={s.uploadBtn} onPress={() => setUploadModal(true)}><Plus size={20} color={INK} strokeWidth={3} /><Text style={s.uploadTxt}>DROP</Text></Pressable>
              </View>
            </View>
            ...
            <View style={s.tabs}>
              <Pressable style={[s.tab, activeTab === 'fresh' && s.tabOn]} onPress={() => setActiveTab('fresh')}><Sparkles size={16} color={activeTab === 'fresh' ? INK : MUTED} /><Text style={[s.tabText, activeTab === 'fresh' && s.tabTextOn]}>FRESH</Text></Pressable>
              <Pressable style={[s.tab, activeTab === 'featured' && s.tabOn]} onPress={() => setActiveTab('featured')}><Flame size={16} color={activeTab === 'featured' ? INK : MUTED} /><Text style={[s.tabText, activeTab === 'featured' && s.tabTextOn]}>FEATURED</Text></Pressable>
            </View>
          </View>
        }
          ...
          return (
            <View style={s.card}>
              <Pressable style={s.media} onPress={() => void watchClip(item)}>
                {item.thumbnail_url ? <Image source={{ uri: item.thumbnail_url }} style={s.fill} contentFit="cover" /> : <View style={s.noThumb}><Camera color={ORANGE} size={42} /><Text style={s.noThumbText}>VIDEO READY</Text></View>}
                <View style={s.scrim} />
                ...
                <View style={s.mediaCopy}><Text style={s.clipTitle} numberOfLines={2}>{item.title || item.trick_name || 'Skate clip'}</Text><Text style={s.byline}>@{item.profiles?.username || 'skater'}</Text></View>
              </Pressable>
              ...
              <View style={s.info}>
                ...
                <View style={s.actions}>
                  <Pressable style={[s.action, liked && s.actionLiked]} onPress={() => void toggleLike(item)}><Heart color={liked ? INK : ORANGE} fill={liked ? INK : 'transparent'} size={18} /><Text style={[s.actionText, liked && s.actionTextLiked]}>{item.likes}</Text></Pressable>
                  <View style={s.action}><Eye color={MUTED} size={18} /><Text style={s.actionText}>{item.views}</Text></View>
                  <Pressable style={s.watch} onPress={() => void watchClip(item)}><Play color={INK} fill={INK} size={13} /><Text style={s.watchText}>WATCH</Text><ArrowUpRight color={INK} size={14} /></Pressable>
                </View>
              </View>
            </View>
          );
        }}
        ListEmptyComponent={<Pressable style={s.empty} onPress={() => setUploadModal(true)}><View style={s.emptyIcon}><Upload color={INK} size={28} /></View><Text style={s.emptyTitle}>{activeTab === 'featured' ? 'NO FEATURED CLIPS YET' : 'THE FEED IS EMPTY'}</Text><Text style={s.emptyText}>{activeTab === 'featured' ? 'Switch to Fresh or post something worth featuring.' : 'Be the first real clip on the scene.'}</Text></Pressable>}
      />

      <Modal visible={uploadModal} transparent animationType="slide" onRequestClose={() => setUploadModal(false)}>
        <View style={s.modalShade}>
          <View style={s.modalSheet}>
            <View style={s.modalHeader}><View><Text style={s.kicker}>DROP A CLIP</Text><Text style={s.modalTitle}>Post to SkateTV</Text></View><Pressable style={s.close} onPress={() => setUploadModal(false)}><X color={PAPER} size={18} /></Pressable></View>
            <ScrollView contentContainerStyle={s.modalContent} keyboardShouldPersistTaps="handled">
              <Pressable style={[s.videoPicker, uploadVideo && s.videoPickerReady]} onPress={() => void pickVideo()}><Upload color={uploadVideo ? INK : ORANGE} size={29} /><Text style={[s.videoPickerTitle, uploadVideo && s.videoPickerReadyText]}>{uploadVideo ? 'VIDEO LOCKED IN' : 'CHOOSE SKATE CLIP'}</Text><Text style={[s.videoPickerSub, uploadVideo && s.videoPickerReadySub]}>Up to 60 seconds</Text></Pressable>
              ...
              <Pressable style={[s.postButton, (!uploadVideo || uploading) && s.postButtonDisabled]} onPress={() => void submitClip()} disabled={!uploadVideo || uploading}>{uploading ? <ActivityIndicator color={INK} /> : <><Upload color={INK} size={18} /><Text style={s.postButtonText}>POST REAL CLIP</Text></>}</Pressable>
            </ScrollView>
          </View>
        </View>
      </Modal>
...
function Input({ label, placeholder, value, onChangeText }: { label: string; placeholder: string; value: string; onChangeText: (value: string) => void }) {
  return <View style={s.inputBlock}><Text style={s.inputLabel}>{label}</Text><TextInput style={s.input} placeholder={placeholder} placeholderTextColor="#626B78" value={value} onChangeText={onChangeText} /></View>;
}

function Stat({ color, value, label }: { color: string; value: number; label: string }) {
  return <View style={[s.stat, { backgroundColor: color }]}><Text style={s.statValue}>{value.toLocaleString()}</Text><Text style={s.statLabel}>{label}</Text></View>;
}
...
const s = StyleSheet.create({
  ...
  videoPickerReadyText: { color: INK },
  videoPickerSub: { color: MUTED, fontSize: 9 },
  videoPickerReadySub: { color: 'rgba(7,8,11,0.62)' },
