...
export default function UploadMediaScreen() {
  ...
  const handleAnalyzeTrick = async () => {
    ...
    if (!namedTrick) { Alert.alert('Add the trick name first', 'Enter the trick you are working on, then get coaching tips.'); return; }
    setAnalyzing(true);
    try { const result = await analyzeTrickVideo(namedTrick, caption || undefined); setAnalysis(result); Alert.alert('Coaching Ready', result.style_notes || `Coaching tips ready for ${namedTrick}.`); }
    catch { Alert.alert('Error', 'Could not get coaching tips right now. Your clip can still be uploaded.'); }
  ...
  const handleUpload = async () => {
    ...
    setUploading(true);
    try {
      if (requiresProofVideo && mediaType !== 'video') throw new Error('This submission requires a video clip. Choose or record a video.');
      const mediaResult = mediaType === 'photo' ? await uploadImage(mediaUri, 'user_photos', user.id) : await uploadVideo(mediaUri, 'user_videos', user.id);
      const media = await saveMediaToDatabase(user.id, mediaResult, { caption: caption || undefined, trickName: trickName.trim() || undefined });
      if (analysis && trickName.trim()) await saveAnalysisResult(user.id, trickName.trim(), analysis, media.url);
  ...
  return (
    <ScrollView className="flex-1 bg-brand-beige dark:bg-gray-900">
      ...
      {!mediaUri ? <View className="p-5"><Text className="text-lg font-bold text-gray-800 dark:text-gray-100 mb-4">Choose Media Type</Text>{!requiresProofVideo ? <><TouchableOpacity className="flex-row items-center bg-white dark:bg-gray-800 p-[18px] rounded-xl mb-3 shadow-sm" onPress={() => handlePickImage(false)}><ImageIcon color="#d2673d" size={28}/><Text className="text-base font-semibold text-gray-800 dark:text-gray-100 ml-4">Photo from Gallery</Text></TouchableOpacity><TouchableOpacity className="flex-row items-center bg-white dark:bg-gray-800 p-[18px] rounded-xl mb-3 shadow-sm" onPress={() => handlePickImage(true)}><Camera color="#d2673d" size={28}/><Text className="text-base font-semibold text-gray-800 dark:text-gray-100 ml-4">Take Photo</Text></TouchableOpacity></> : <Text className="text-sm text-gray-500 dark:text-gray-400 mb-4">This is proof for a real challenge, so a video clip is required.</Text>}<TouchableOpacity className="flex-row items-center bg-white dark:bg-gray-800 p-[18px] rounded-xl mb-3 shadow-sm" onPress={() => handlePickVideo(false)}><Film color="#d2673d" size={28}/><Text className="text-base font-semibold text-gray-800 dark:text-gray-100 ml-4">Video from Gallery</Text></TouchableOpacity><TouchableOpacity className="flex-row items-center bg-white dark:bg-gray-800 p-[18px] rounded-xl mb-3 shadow-sm" onPress={() => handlePickVideo(true)}><VideoIcon color="#d2673d" size={28}/><Text className="text-base font-semibold text-gray-800 dark:text-gray-100 ml-4">Record Video</Text></TouchableOpacity></View> : <View className="p-5"><Text className="text-lg font-bold text-gray-800 dark:text-gray-100 mb-4">Preview</Text>{mediaType === 'photo' ? <Image source={{uri:mediaUri}} style={{width:'100%',height:300,borderRadius:12,backgroundColor:'#000',marginBottom:16}}/> : <Video source={{uri:mediaUri}} style={{width:'100%',height:300,borderRadius:12,marginBottom:16}} useNativeControls resizeMode={ResizeMode.CONTAIN}/>}<Button title="Change Media" onPress={() => {setMediaUri(null);setMediaType(null);setAnalysis(null);}} variant="secondary" size="md" className="mb-5"/><View className="mb-5"><Text className="text-sm font-semibold text-gray-800 dark:text-gray-100 mb-2">Trick Name {bountyId || challengeId ? '(proof target)' : '(optional)'}</Text><TextInput className="bg-white dark:bg-gray-800 rounded-lg p-3 text-base border border-gray-200 dark:border-gray-700 text-gray-800 dark:text-gray-100" placeholder="e.g., Kickflip, Heelflip" placeholderTextColor="#999" value={trickName} onChangeText={setTrickName}/></View>{mediaType === 'video' && !analysis && <TouchableOpacity className={`flex-row bg-purple-600 p-3.5 rounded-lg items-center justify-center mb-4 ${analyzing?'opacity-60':''}`} onPress={handleAnalyzeTrick} disabled={analyzing}>{analyzing ? <><ActivityIndicator color="#fff" size="small"/><Text className="text-white text-[15px] font-bold ml-2">Getting coaching...</Text></> : <><Bot color="#fff" size={20}/><Text className="text-white text-[15px] font-bold ml-2">Get AI Coaching Tips</Text></>}</TouchableOpacity>}{analysis && <Card className="border-l-4 border-l-purple-600 mb-5"><View className="flex-row items-center gap-2 mb-2"><Bot color="#9C27B0" size={18}/><Text className="text-base font-bold text-gray-800 dark:text-gray-100">AI Coaching</Text></View><Text className="text-sm text-gray-500 dark:text-gray-400 mb-2">Coaching for <Text className="font-bold text-purple-600">{trickName.trim()}</Text> · {analysis.difficulty}</Text><Text className="text-sm text-gray-800 dark:text-gray-200 italic mb-2.5">{analysis.style_notes}</Text>{(analysis.tips?.length??0)>0 && <View className="mt-1"><Text className="text-xs font-bold text-gray-500 mb-1">Tips</Text>{analysis.tips.slice(0,3).map((tip:string,index:number)=><Text key={index} className="text-xs text-gray-600 dark:text-gray-300 mb-1">• {tip}</Text>)}</View>}<Text className="text-xs text-gray-500 mt-2">Coaching is based on the trick name you entered. Proof is verified by the bounty/challenge workflow, not by this coaching tool.</Text></Card>}<View className="mb-5"><Text className="text-sm font-semibold text-gray-800 dark:text-gray-100 mb-2">Caption (optional)</Text><TextInput className="bg-white dark:bg-gray-800 rounded-lg p-3 text-base border border-gray-200 dark:border-gray-700 text-gray-800 dark:text-gray-100" placeholder="Say something about this..." placeholderTextColor="#999" value={caption} onChangeText={setCaption} multiline numberOfLines={3} style={{height:80,textAlignVertical:'top'}}/></View><Button title={submitButtonTitle} onPress={handleUpload} variant="primary" size="lg" className="bg-brand-green mt-2.5" disabled={uploading}/></View>}
    </ScrollView>
  );
}
