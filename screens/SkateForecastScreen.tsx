...
export default function SkateForecastScreen() {
  ...
  const loadForecasts = async () => {
    try {
      ...
      if (status !== 'granted') {
        setForecasts([]);
        setMessage('Location permission is required to show real weather for skate spots near you.');
        return;
      }
      ...
      try {
        ...
        if (!lastKnown) {
          setForecasts([]);
          setMessage('Your current location could not be verified. Pull down to try again.');
          return;
        }
      }
      ...
      if (!location) {
        setForecasts([]);
        setMessage('Your current location could not be verified.');
        return;
      }
