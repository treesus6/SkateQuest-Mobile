  ...
  const loadForecasts = async () => {
    try {
      ...
      for (const spot of spots.slice(0, 5)) {
        ...
        const bustRisk = conditions?.bust_risk ?? 20;
        ...
        results.push({
          ...
          rec:
            score >= 80
              ? 'Perfect day to skate. Low bust risk, no rain.'
              : score >= 60
                ? 'Decent conditions. Watch for afternoon showers.'
    ...
    <SafeAreaView style={s.container}>
      ...
      >
        <View style={s.header}>
          ...
          {locationMessage ? <Text style={s.locationMessage}>{locationMessage}</Text> : null}
        </View>

        <View style={s.cards}>
          {forecasts.length > 0 ? (
            forecasts.map((item, i) => (
              <View key={`${item.park_name}-${i}`} style={s.card}>
                ...
                <View style={s.metrics}>
                  <View style={s.metric}>
                    ...
                    <Text style={[s.metricValue, { color: item.rain > 40 ? '#ef4444' : '#4ade80' }]}>
                      {item.rain}%
                    </Text>
                  </View>
                  <View style={s.metric}>
                    ...
                    <Text
                      style={[
                        s.metricValue,
                        { color: item.bust > 50 ? '#ef4444' : item.bust > 30 ? '#fbbf24' : '#4ade80' },
                      ]}
                    >
                      {item.bust}%
                    </Text>
        ...
        <View style={s.legend}>
          ...
          <Text style={s.legendText}>
            We combine weather conditions, chance of rain, reported bust risk, and time of day to
            give each park a daily score. Higher = better day to skate.
          </Text>
        </View>
      </ScrollView>
    </SafeAreaView>
...
const s = StyleSheet.create({
  ...
  locationMessage: { color: '#FBBF24', fontSize: 12, lineHeight: 18, marginTop: 10 },
